...
go 
CREATE FUNCTION [FN_StudentUpcoming_installment_deadline]
     (@student_ID int)
   RETURNs date
  Begin
...
CREATE FUNCTION [FN_StudentUpcoming_installment]
     (@student_ID int)
   RETURNs table
  return (Select top 1 Installment.payment_id, Installment.amount, Installment.startdate, Installment.deadline, Installment.status
  from Installment inner join Payment
  on Payment.payment_id = Installment.payment_id and Payment.student_id = @student_ID and Installment.status='notpaid'
  where Installment.deadline > CURRENT_TIMESTAMP
  Order by Installment.deadline ASC)
...
BEGIN
    ...
    IF dbo.FN_StudentCheckFMEligibility(@StudentID, @courseID) = 1
    BEGIN
...
GO
------------------------------------------------------------------------------------------
----------get satudent's name from id-------
Go
Create PROC [Procedures_GetStudentName]
@StudentID int
AS
...
insert into course(name,semester) values ('Introduction to Computer Science',1)
insert into course(name,semester) values ('Introduction to Programming',2)
insert into course(name,semester) values ('Data Structures',3)
insert into course(name,semester) values ('Algorithms',7)

insert into makeup_exam(date,type,course_id) values ('2020-12-12','First_makeup',21)
insert into makeup_exam(date,type,course_id) values ('2020-12-15','First_makeup',22)
insert into makeup_exam(date,type,course_id) values ('2020-12-18','First_makeup',23)
insert into makeup_exam(date,type,course_id) values ('2020-12-21','First_makeup',24)

insert into Instructor(instructor_id,name) values (1,'Ahmed Mohammed')
insert into Instructor(instructor_id,name) values (2,'Mohamed Ali')
insert into Instructor(instructor_id,name) values (3,'Ali Shawky')
insert into Instructor(instructor_id,name) values(4,'Hassan Ibrahim')

insert into Slot(slot_id,location,time,day,course_id,instructor_id) values (1,'H7','10:00','Sunday',22,1)
insert into Slot(slot_id,location,time,day,course_id,instructor_id) values (2,'H12','11:45','Monday',22,2)
insert into Slot(slot_id,location,time,day,course_id,instructor_id) values (3,'H13','1:45','Tuesday',22,1)
insert into Slot(slot_id,location,time,day,course_id,instructor_id) values (4,'H10','3:45','Wednesday',23,1)


print dbo.FN_studentLogin(15,'123')

SELECT dbo.FN_StudentCheckFMEligibility(1,22)

exec  Procedures_StudentaddMobile 1,'1111'

exec [Procedures_GetStudentName] 1